...
# A quadratic
    M2 = np.zeros([n, s2])
    M2[0, :] = 5*np.array(rnd.rand(1, s2)-0.5*np.ones((1, s2)))
    M2[1, :] = M2[0, :]*M2[0, :]

    Mtemp = np.zeros([n, s1])
    Mtemp[0, :] = 2.5*np.array(rnd.rand(1, s1)-0.5*np.ones((1, s1)))
    ...
    plt.legend()
    # plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
    plt.savefig('registration-overlap-sigma1.eps', format='eps')
    plt.show()
